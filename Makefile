...
.PHONY: all clean test test-update test-advanced test-price-node test-ratio test-delivered-work test-price-dynamics test-node-types test-schema test-python demo demo-python plot-demo directories swift-build swift-test swift-clean dist \
...
test-linux: container-image-linux
	$(CRUN) $(IMAGE_LINUX) sh -c 'make clean && make CC=gcc all && make CC=gcc test && make CC=gcc test-advanced && make CC=gcc test-node-types && make CC=gcc test-price-node test-ratio test-delivered-work test-price-dynamics'
