...
\begin{abstract}
A \gssk{} model is a system of ordinary differential equations describing flows of energy, matter and money through a network of storages. Over that same network Odum's emergy accounting values flows by their embodied energy, under an algebra of its own. A general ledger is a third thing again: a set of discrete, exactly balancing entries recognised under a body of accounting standards. The premise of this document is that one artefact can serve all three purposes --- that a model can be simultaneously a faithful Odum simulation, an emergy evaluation, and a compliant set of books --- and that this is achievable without deforming any of them. We identify seven specific points at which the dynamics and the standards genuinely diverge, distinguish them from the larger set of points at which they merely differ in vocabulary, and resolve each against the IFRS/AASB standards. Emergy is treated as a peer accounting rather than as a unit conversion, because its algebra is deliberately non-additive at co-products and therefore cannot produce a ledger entry even in principle --- an exclusion that rests on arithmetic rather than on the closed list of IFRS measurement bases. The organising resolution is that the trajectory is \emph{evidence for postings} rather than the book of record itself: a solver balances to tolerance, a ledger balances exactly, and the two remain distinct artefacts joined by an explicit, specified posting step. We show that this preserves the continuity Odum's method requires and the exactness the standards require, at a cost that is stated rather than hidden. We also record that the emergy accounting is not yet settled on its own terms: where the classical and incipient calculi disagree, which of the two answers to report is a measurement choice rather than an implementation detail. Four defects found in the kernel while establishing this are reported with the measurements that revealed them.
\end{abstract}
...
\gssk{} today implements the additive subset. The quality pass propagates transformity by solving a linear balance across each node's outflows, which is the split rule; the co-product rule is not implemented, and \code{docs/giannantoni\_assessment.md} records the full algebra as deferred work. This is worth knowing because it explains why the collision described in Section~\ref{sec:valuation} has not yet been felt in practice: the emergy accounting currently running inside the kernel is the conservative half of the real one.
...
\textbf{The cost} is that a model carries two valuations that must not be mixed, and the discipline of keeping them apart falls partly on the modeller. A statement that presents emergy alongside monetary amounts must label which is which. The cost is currently smaller than it will become: \gssk{} implements only the additive half of emergy algebra today, so the two valuations still compose alike, and it is when the co-product rule is implemented that the separation will have to be enforced rather than merely observed.
